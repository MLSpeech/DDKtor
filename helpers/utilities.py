...
def get_features_batch(source):
    batch_in = torch.Tensor(source).float().squeeze()
    return batch_in

def basic_hierarchy_dict(basic_hierarchy_file):
    files_basic_hierarchy_list = open(basic_hierarchy_file, "r").readlines()
    hierarchy_dict = {}
    #0:./data/raw/dir2/1.wav
    for file_line in files_basic_hierarchy_list[2:]:
        file_key, file_dir = file_line.strip("\n").split(":")
        hierarchy_dict[file_key] = file_dir
    return hierarchy_dict

def get_hierarchy_path(hierarchy_dict, filename):
    prev_full_name = hierarchy_dict[filename]
    file_hierarchy = prev_full_name.split("/")[3:]
    prev_filename = file_hierarchy[-1]
    prev_filename = prev_filename.replace(".WAV",".wav")
    prev_hierarchy = "/".join(file_hierarchy[:-1])
    return prev_hierarchy, prev_filename
